...
__publicField(Quill, "version", false ? "dev" : "2.0.0");
...
var image_default2 = LoadingImage;
...
var ImageUploader = class {
  ...
  handleDrop(evt) {
    if (evt.dataTransfer && evt.dataTransfer.files && evt.dataTransfer.files.length) {
      ...
      if (document.caretRangeFromPoint) {
        const selection = document.getSelection();
        const range2 = document.caretRangeFromPoint(evt.clientX, evt.clientY);
        if (selection && range2) {
        ...
        const selection = document.getSelection();
        const range2 = document.caretPositionFromPoint(evt.clientX, evt.clientY);
        if (selection && range2) {
          selection.setBaseAndExtent(
            range2.offsetNode,
            range2.offset,
  ...
  insertToEditor(url) {
    ...
    this.quill.deleteText(range2.index, lengthToDelete, "user");
    this.quill.insertEmbed(range2.index, "image", `${url}`, "user");
    range2.index++;
    this.quill.setSelection(range2, "user");
  }
